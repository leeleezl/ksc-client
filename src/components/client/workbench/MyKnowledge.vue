<template>
    <div>
        ...
        <el-card>
            ...
            <el-row :gutter="20">
                <el-col :span="7">
                    <el-input placeholder="请输入内容" v-model="queryInfo.queryKey" clearable @clear="getMyKnowledgeList()">
                        <el-button slot="append" icon="el-icon-search" @click="getMyKnowledgeList()"></el-button>
                    </el-input>
                </el-col>
                <!-- <el-col :span="4">
            ...
            <el-table :data="myKnowledgeList">
                ...
                <el-table-column label="操作" width="190px">
                    <template slot-scope="scope">
                        <!-- 详情按钮 -->
                        <el-button type="primary" icon="el-icon-view" size="mini" @click="showKnowledgeDialog(scope.row.id)"></el-button>
                        <el-button type="warning" icon="el-icon-view" size="mini" @click="showEditDialog(scope.row.id)"></el-button>
                        <el-button type="danger" icon="el-icon-delete" size="mini" @click="deleteMyKnowledge(scope.row.id)"></el-button>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination
...
export default {
    data() {
        return {
            ...
            editForm: {},
            getStatus: [
                { dictValue: 0, dictLable: '审核未通过' },
                { dictValue: 1, dictLable: '审核中' },
                { dictValue: 2, dictLable: '等待人工审核' },
                { dictValue: 3, dictLable: '审核通过' }
            ],
            Status: [
                '审核未通过',
                '审核中',
    ...
    methods: {
        async getMyKnowledgeList() {
            const {data: res} = await this.$http.post('/api/knowledge/myKnowledge', this.queryInfo)
